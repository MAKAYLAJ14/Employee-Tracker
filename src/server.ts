...
function employees(answers: any) {
    const { id } = answers;
    return id;
}
...
const viewEmployeesByDepartment = async () => {
  try {
    ...
    const id = employees(dep_id.rows[0]);
...
async function updateEmployeeManager(){
  try {
    ...
    const man_id = employees(newID.rows[0]);
...
async function deleteDepartment(){
  try{
    ...
    const delId = employees(id.rows[0]);
